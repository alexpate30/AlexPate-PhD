...
load("R_out_C6/generate_model_agetime_female.RData")
load("R_out_C6/simulation_functions_agetime_models_female.RData")
load("R_out_C6/generate_risk_range_for_each_age_agetime_female.RData")
load("R_out_C6/statin_discontinuation_rates_for_sim_continuous_multiple_treatment_periods_rates_no1pres_agemod.RData")
  ...
  for (i in c(4,6,8)){
    ...
    for (j in max(1,round(min.max.prange.age.agetime[i,3])):min(20,round(min.max.prange.age.agetime[i,6]))){
      ...
      for (k in 1:(90-ages[i])){
        data.out[(j-max(1,round(min.max.prange.age.agetime[i,3]))+1),k] <- run.simulation.agetime.CPRDdisc.cont.multiple.treat(j,ages[i],k,0.65,
...
## Run simulation seperately for each set of discontinuation rates
res.multiple.treatment.periods.sixth <- create.output(disc1.extrapolated.list[[2]],
                                                      disc2.extrapolated.list[[2]],
                                                      disc3.extrapolated.list[[2]],
                                                      restart1.extrapolated,
                                                      restart2.extrapolated)

print("done 1")

res.multiple.treatment.periods.third <- create.output(disc1.extrapolated.list[[3]],
                                                      disc2.extrapolated.list[[3]],
                                                      disc3.extrapolated.list[[3]],
                                                      restart1.extrapolated,
                                                      restart2.extrapolated)

print("done 2")

res.multiple.treatment.periods.half <- create.output(disc1.extrapolated.list[[4]],
                                                     disc2.extrapolated.list[[4]],
                                                     disc3.extrapolated.list[[4]],
                                                     restart1.extrapolated,
                                                     restart2.extrapolated)

print("done 3")

res.multiple.treatment.periods.nodisc <- create.output(disc1.extrapolated.list[[5]],
                                                       disc2.extrapolated.list[[5]],
                                                       disc3.extrapolated.list[[5]],
                                                       restart1.extrapolated,
                                                       restart2.extrapolated)

print("done 4")

res.multiple.treatment.periods <- create.output(disc1.extrapolated.list[[1]],
                                                disc2.extrapolated.list[[1]],
                                                disc3.extrapolated.list[[1]],
                                                restart1.extrapolated,
                                                restart2.extrapolated)

rm(anal.dat,basehaz.model,comp.data, agetime.model.covar, agetime.model)

save.image("R_out_C6/runsim_female_RR065_discadj_no1pres.RData")
print("image saved")

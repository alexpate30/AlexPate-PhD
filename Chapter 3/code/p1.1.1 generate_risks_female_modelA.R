##### This program will generate the risks for each patient according to model A (female cohort)
##### This program will generate the risks for each patient according to model A (female cohort)
##### This program will generate the risks for each patient according to model A (female cohort)
...
create_risks<-function(model){
  ...
  C<-model$var
  S<-model.matrix(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMIfrac1 + BMIfrac2 + Atrialfib+ 
                          Cholesterol_HDL_Ratio + CKD45 + Ethnicity + Famhis_lstrict + Hypertension+ 
                          RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                          age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                          age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + 
                          age.Townsend.2 + age.Townsend.3 + age.Townsend.4 + age.BMI + age.SBP, data=mean.dat.test)
  S<-S[,-1]
  p<-(S%*%B)[,1]

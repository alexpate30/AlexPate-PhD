...
statin.cohort.no1pres <- read.table("data/statin_users_cohort_no1pres.csv", sep="," , header=TRUE)
...
statin.cohort.no1pres$index_date_D_r <- statin.cohort.no1pres$stat_init_r

## Creaet new dataset of interest
statin.cohort.no1pres <- merge(anal.dat,statin.cohort.no1pres)
statin.cohort.no1pres <- arrange(statin.cohort.no1pres,patid)
...
statin.cohort.no1pres$age.init <- statin.cohort.no1pres$age

# Now carry it over from the first value for each patient, whenever patient id is the same
for (i in 2:nrow(statin.cohort.no1pres)){
  if (statin.cohort.no1pres$patid[i] == statin.cohort.no1pres$patid[i-1]){
    statin.cohort.no1pres$age.init[i] <- statin.cohort.no1pres$age.init[i-1]
  }}

## Now remove entries that start before 85
statin.cohort.no1pres <- statin.cohort.no1pres[statin.cohort.no1pres$age.init < 85, ]

##### RENAME DATASET BACK TO statin.cohort SO I CAN RE-USE CODE ######
statin.cohort <- statin.cohort.no1pres
...
# Finally double check how many entries are left when we just keep the first entry for each patient
...
disc_sum[disc_sum$time %in% round(seq(182.625,3652.5,182.625)),]
restart_sum[restart_sum$time %in% round(seq(182.625,3652.5,182.625)),]
disc2_sum[disc2_sum$time %in% round(seq(182.625,3652.5,182.625)),]
restart2_sum[restart2_sum$time %in% round(seq(182.625,3652.5,182.625)),]
disc3_sum[disc3_sum$time %in% round(seq(182.625,3652.5,182.625)),]
restart3_sum[restart3_sum$time %in% round(seq(182.625,3652.5,182.625)),]
disc4_sum[disc4_sum$time %in% round(seq(365.25,5478.75,365.25)),]
...
anal.dat.all.dedup$age.init.frac1 <- (anal.dat.all.dedup$age.init/100)^2
anal.dat.all.dedup$age.init.frac2 <- (anal.dat.all.dedup$age.init/100)^3
...
## Derive these frac polys in the analysis dataset
restart.dat2$age.init.frac1 <- (restart.dat2$age.init/100)
restart.dat2$age.init.frac2 <- ((restart.dat2$age.init/100)^2)
...
disc2_create_fracs <- function(x){
  return(data.frame("age.init.frac1" = (x/100), "age.init.frac2" = (x/100)^2))
}
...
## Derive these frac polys in the analysis dataset
restart.dat3$age.init.frac1 <- (restart.dat3$age.init/100)
restart.dat3$age.init.frac2 <- (restart.dat3$age.init/100)*log(restart.dat3$age.init/100)
...
## Derive these frac polys in the analysis dataset
restart.dat4$age.init.frac1 <- (restart.dat4$age.init/100)^0.5
restart.dat4$age.init.frac2 <- (restart.dat4$age.init/100)
...
disc3_create_fracs <- function(x){
  return(data.frame("age.init.frac1" = (x/100)^0.5, "age.init.frac2" = (x/100)))
}
...
save.image("R_out_C6/survival_curves_no1pres_1year_restricted_agemod_FAKE.RData")
##### This program will generate Integrate Brier Score using model B, female cohort
...
get.IBS<-function(in1,in2){temp<-pec(in1,formula=Surv(CVD_time,CVD_cens_R) ~ agefrac1 + agefrac2 + BMIfrac1 + BMIfrac2 +  Atrialfib+ Atypical_antipsy_med+ 
                                       Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + Ethnicity + Famhis_lstrict + Hypertension+ 
                                       Migraine+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                                       age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                       age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 +
                                       age.Townsend.3 + age.Townsend.4 + age.BMI + age.SBP, 
                                     data=mean.dat.test,
...
setwd("B")
save.image("generate_discrim_measures_female_IBS_modelB.RData")

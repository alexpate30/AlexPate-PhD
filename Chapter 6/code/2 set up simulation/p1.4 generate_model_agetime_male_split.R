...
## THIS MODEL IS USED FOR CALIBRATION, 400,000 patients (~20%) are extracted and used for validation rather than development ##
## THIS MODEL IS USED FOR CALIBRATION, 400,000 patients (~20%) are extracted and used for validation rather than development ##
...
load("R_out_C6/imputed_datasets_loaded_male_FAKE.RData")
...
## Now want to split the model into development and validation
set.seed(60421)
random.rows <- sample(1:dim(comp.data)[1],400000,replace = F)
comp.data.devel <- comp.data[-random.rows, ]
comp.data.valid <- comp.data[random.rows, ]

## Try fitting the model
#agetime.model <- coxph(Surv(time = age.time.start,time2 = age.time.end, CVD_cens_R) ~ 1, data=comp.data.devel)
...
agetime.model.covar <- coxph(Surv(time = age.time.start,time2 = age.time.end, CVD_cens_R) ~ BMI + Atrialfib+ Atypical_antipsy_med+ 
                               ...
                               Migraine+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE, data=comp.data.devel)
...
save.image("R_out_C6/generate_model_agetime_male_split_FAKE.RData")
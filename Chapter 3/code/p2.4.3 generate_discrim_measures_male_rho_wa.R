### In this program I will calculate rho_wa for models A, B, C and D (male cohort)
...
fit_data_parallel.1<-(foreach(input=list(long_data_parallel_devel[[1]],long_data_parallel_devel[[2]],long_data_parallel_devel[[3]],long_data_parallel_devel[[4]],long_data_parallel_devel[[5]],
                      ...
                      %dopar%{coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ 
                                      ...
                                      age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 +
                                      age.Townsend.4 + age.BMI + age.SBP, data=input)
                      })
...
pred.model.1[[1]] <- predict(fit_data_parallel.1[[1]],newdata = mean.dat.test)
pred.model.1[[2]] <- predict(fit_data_parallel.1[[2]],newdata = mean.dat.test)
pred.model.1[[3]] <- predict(fit_data_parallel.1[[3]],newdata = mean.dat.test)
pred.model.1[[4]] <- predict(fit_data_parallel.1[[4]],newdata = mean.dat.test)
pred.model.1[[5]] <- predict(fit_data_parallel.1[[5]],newdata = mean.dat.test)
pred.model.1[[6]] <- predict(fit_data_parallel.1[[6]],newdata = mean.dat.test)
pred.model.1[[7]] <- predict(fit_data_parallel.1[[7]],newdata = mean.dat.test)
pred.model.1[[8]] <- predict(fit_data_parallel.1[[8]],newdata = mean.dat.test)
pred.model.1[[9]] <- predict(fit_data_parallel.1[[9]],newdata = mean.dat.test)
pred.model.1[[10]] <- predict(fit_data_parallel.1[[10]],newdata = mean.dat.test)

print("first half of linear predictors done")

pred.model.2 <- vector("list",10)
pred.model.2[[1]] <- predict(fit_data_parallel.2[[1]],newdata = mean.dat.test)
pred.model.2[[2]] <- predict(fit_data_parallel.2[[2]],newdata = mean.dat.test)
pred.model.2[[3]] <- predict(fit_data_parallel.2[[3]],newdata = mean.dat.test)
pred.model.2[[4]] <- predict(fit_data_parallel.2[[4]],newdata = mean.dat.test)
pred.model.2[[5]] <- predict(fit_data_parallel.2[[5]],newdata = mean.dat.test)
pred.model.2[[6]] <- predict(fit_data_parallel.2[[6]],newdata = mean.dat.test)
pred.model.2[[7]] <- predict(fit_data_parallel.2[[7]],newdata = mean.dat.test)
pred.model.2[[8]] <- predict(fit_data_parallel.2[[8]],newdata = mean.dat.test)
pred.model.2[[9]] <- predict(fit_data_parallel.2[[9]],newdata = mean.dat.test)
pred.model.2[[10]] <- predict(fit_data_parallel.2[[10]],newdata = mean.dat.test)

print("second half of linear predictors done")

rho_wa.1<- vector("list",10)
for (i in 1:10){
  rho_wa.1[[i]] <- var(pred.model.1[[i]])/(var(pred.model.1[[i]])+1)
}

rho_wa.2<- vector("list",10)
for (i in 1:10){
  rho_wa.2[[i]] <- var(pred.model.2[[i]])/(var(pred.model.2[[i]])+1)
}

rho_wa.A<-c(rho_wa.1,rho_wa.2)
...
rho_wa.D<-c(rho_wa.1,rho_wa.2)


print("removeing before saving")
rm(list=setdiff(ls(),list("rho_wa.A","rho_wa.B","rho_wa.C","rho_wa.D")))
setwd("B")
save.image("generate_discrim_measures_male_rho_wa.RData")

print("image saved")

mean(unlist(rho_wa.A))
mean(unlist(rho_wa.B))
mean(unlist(rho_wa.C))
mean(unlist(rho_wa.D))
mean(unlist(rho_wa.E))
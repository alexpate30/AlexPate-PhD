### In this program I will calculate UnoC for models A, B, C and D (male cohort)
...
cl <- makeCluster(11)
registerDoParallel(11)
UnoC.1<-(foreach(input=pred.model.1, .combine=list, .multicombine=TRUE, 
                 .packages=c("dplyr","mice","tidyr","survival"))
         %dopar%{UnoC<-UnoC(Surv.rsp=Surv(anal.dat.test$CVD_time,anal.dat.test$CVD_cens_R),Surv.rsp.new=Surv(anal.dat.test$CVD_time,anal.dat.test$CVD_cens_R),lpnew=input)
                 UnoC
         })
stopCluster(cl)
...
cl <- makeCluster(11)
registerDoParallel(11)
UnoC.2<-(foreach(input=pred.model.2, .combine=list, .multicombine=TRUE, 
                 .packages=c("dplyr","mice","tidyr","survival"))
         %dopar%{UnoC<-UnoC(Surv.rsp=Surv(anal.dat.test$CVD_time,anal.dat.test$CVD_cens_R),Surv.rsp.new=Surv(anal.dat.test$CVD_time,anal.dat.test$CVD_cens_R),lpnew=input)
                 UnoC
         })
...
rm(list=setdiff(ls(),list("UnoC.A","UnoC.B","UnoC.C","UnoC.D")))
setwd("B")
save.image("generate_discrim_measures_male_UnoC.RData")

##### This program will generate the risks for each patient in the 2016 cohort, for models A, B, C and D (male cohort)
##### This program will generate the risks for each patient in the 2016 cohort, for models A, B, C and D (male cohort)
##### This program will generate the risks for each patient in the 2016 cohort, for models A, B, C and D (male cohort)
...
setwd("A")
load("imputed_datasets_loaded_male_C.RData")
...
convert_2016<-function(model){
  ...
  model$age<-(model$age+mean.age - mean.age.orig + 16.9)/10
...
for (i in 1:20){long_data_parallel[[i]]<-convert_male(long_data_parallel[[i]])}
...
for (i in 1:10){fit_data_parallel.1[[i]] <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ 
                                                    ...
                                                    age.Townsend.4 + age.BMI + age.SBP, data=long_data_parallel_devel[[i]])}
...
for (i in 1:10){fit_data_parallel.2[[i]] <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ 
                                                    Cholesterol_HDL_Ratio + CKD45 + Ethnicity + Famhis_lstrict + Hypertension+ 
                                                    RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                                                    age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 +
                                                    age.Townsend.4 + age.BMI + age.SBP, data=long_data_parallel_devel[[(i+10)]])}
...
create_risks<-function(model){
  ...
  C<-model$var
  S<-model.matrix(Surv(patid,.imp, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ 
                    Cholesterol_HDL_Ratio + CKD45 + Ethnicity + Famhis_lstrict + Hypertension+ 
                    RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                    age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 +
                    age.Townsend.4 + age.BMI + age.SBP, data=mean.dat)
  S<-S[,-1]
  p<-(S%*%B)[,1]
...
fit_data_parallel.1 <- vector("list",10)
for (i in 1:10){fit_data_parallel.1[[i]] <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ Atypical_antipsy_med+ 
...
fit_data_parallel.2 <- vector("list",10)
for (i in 1:10){fit_data_parallel.2[[i]] <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ Atypical_antipsy_med+ 
                                                    Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
                                                    Migraine+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                                                    age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 +
...
create_risks<-function(model){
  ...
  B<-model$coefficients
  C<-model$var
  S<-model.matrix(Surv(patid,.imp, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ Atypical_antipsy_med+ 
                    Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
                    Migraine+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                    age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 +
...
setwd("B")
save.image("risks_male_cohort2016.RData")
...
## Analyse the imputed datasets
fit_data_parallel.1 <- vector("list",10)
for (i in 1:10){fit_data_parallel.1[[i]]<-coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Alcohol_abuse+ Anxiety+ Atrialfib+ Atypical_antipsy_med+ 
...
fit_data_parallel.2 <- vector("list",10)
for (i in 1:10){fit_data_parallel.2[[i]]<-coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Alcohol_abuse+ Anxiety+ Atrialfib+ Atypical_antipsy_med+ 
                                                  Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + LVH + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
                                                  Medrec1yo50+ Migraine+ Prescrip1yo50+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                                                  age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                                  age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + 
...
create_risks<-function(model){
  ...
  B<-model$coefficients
  C<-model$var
  S<-model.matrix(Surv(patid,.imp, type='right') ~ agefrac1 + agefrac2 + BMI + Alcohol_abuse+ Anxiety+ Atrialfib+ Atypical_antipsy_med+ 
                    Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + LVH + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
                    Medrec1yo50+ Migraine+ Prescrip1yo50+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                    age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + 

  ...
  for (i in c(4,6,8)){
    ...
    for (j in max(1,round(min.max.prange.age.agetime[i,3])):min(20,round(min.max.prange.age.agetime[i,6]))){
      ...
      for (k in 1:(90-ages[i])){
        data.out[(j-max(1,round(min.max.prange.age.agetime[i,3]))+1),k] <- run.simulation.agetime.CPRDdisc.cont.multiple.treat(j,ages[i],k,0.6,
...
save.image("R_out_C6/runsim_female_RR06_discadj_no1pres.RData")
print("image saved")

##### This program will generate Integrate Brier Score using model A, female cohort
##### The output also contains IBS for null model, required to calculate R2_IBS
...
fit_data_parallel.2<-(foreach(input=list(long_data_parallel_devel[[11]],long_data_parallel_devel[[12]],long_data_parallel_devel[[13]],long_data_parallel_devel[[14]],long_data_parallel_devel[[15]],
                      ...
                      %dopar%{coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMIfrac1 + BMIfrac2 +  Atrialfib+ 
                                      ...
                                      RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                                      age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                      age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 + age.Townsend.4 + age.BMI + age.SBP, data=input)
                      })
end_time_fit_parallel <- Sys.time()
diff_fit_parallel <- start_time_fit_parallel - end_time_fit_parallel
stopCluster(cl)
...
## Define function used to calculate IBS

## Note documentation online now says the 'ibs' function is actually crps
get.IBS<-function(in1,in2){temp<-pec(in1,formula=Surv(CVD_time,CVD_cens_R) ~ agefrac1 + agefrac2 + BMIfrac1 + BMIfrac2 +  Atrialfib+ 
                                       Cholesterol_HDL_Ratio + CKD45 + Ethnicity + Famhis_lstrict + Hypertension+ 
                                       RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                                       age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                       age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 + age.Townsend.4 + age.BMI + age.SBP, 
                                     data=mean.dat.test,
                                     traindata=in2,times=3652,cens.model = "cox",splitMethod="none")
                           out<-ibs(temp,what="AppErr",times=3652,start=0)
                           return(out)}

IBS <- vector("list",20)
for (i in 1:10){IBS[[i]]<-get.IBS(fit_data_parallel.1[[i]],long_data_parallel_devel[[i]])
print(i)}
for (i in 11:20){IBS[[i]]<-get.IBS(fit_data_parallel.2[[(i-10)]],long_data_parallel_devel[[i]])
print(i)}


print("remove excess before saving")
rm(list=setdiff(ls(),list("IBS")))
setwd("B")
save.image("generate_discrim_measures_female_IBS_modelA.RData")
print('model A saved')

### In this program I will calculate R2_PM for models A, B, C and D (male cohort)
...
R2_PM.1<- vector("list",10)
for (i in 1:10){
  R2_PM.1[[i]] <- var(pred.model.1[[i]])/(var(pred.model.1[[i]])+(pi^2)/6)
}

R2_PM.2<- vector("list",10)
for (i in 1:10){
  R2_PM.2[[i]] <- var(pred.model.2[[i]])/(var(pred.model.2[[i]])+(pi^2)/6)
}

R2_PM.A<-c(R2_PM.1,R2_PM.2)
...
R2_PM.D<-c(R2_PM.1,R2_PM.2)



print("removeing before saving")
rm(list=setdiff(ls(),list("R2_PM.A","R2_PM.B","R2_PM.C","R2_PM.D")))
setwd("B")
save.image("generate_discrim_measures_male_R2_PM.RData")

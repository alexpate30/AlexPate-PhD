##### This program will generate the risks for each patient in the 2016 cohort, for models E and F (male cohort)
##### This program will generate the risks for each patient in the 2016 cohort, for models E and F (male cohort)
##### This program will generate the risks for each patient in the 2016 cohort, for models E and F (male cohort)
...
create_risks<-function(model,frailties){
  ...
  S<-model.matrix(Surv(patid,.imp, type='right') ~ agefrac1 + agefrac2 + BMI + Alcohol_abuse+ Anxiety+ Atrialfib+ Atypical_antipsy_med+ 
                    Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + HIV + LVH + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
...
setwd("B")
save.image("risks_male_cohort2016_EandF.RData")
print('image saved')
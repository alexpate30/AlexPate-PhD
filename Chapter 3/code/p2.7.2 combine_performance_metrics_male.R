...
load("discrim_mesaures_male_modelB.RData")
D_RS.B<-D_RS.A
R2_D_RS.B<-R2_D_RS.A
GH_C_v1.B<-GH_C_v1.A
GH_C_v2.B<-GH_C_v2.A
...
load("discrim_mesaures_male_modelC.RData")
D_RS.C<-D_RS.A
R2_D_RS.C<-R2_D_RS.A
GH_C_v1.C<-GH_C_v1.A
GH_C_v2.C<-GH_C_v2.A
...
load("discrim_mesaures_male_modelD.RData")
D_RS.D<-D_RS.A
R2_D_RS.D<-R2_D_RS.A
GH_C_v1.D<-GH_C_v1.A
GH_C_v2.D<-GH_C_v2.A
...
load("discrim_mesaures_male_modelE.RData")
D_RS.E<-D_RS.A
R2_D_RS.E<-R2_D_RS.A
HarrelsC.E<-HarrelsC.A

load("discrim_mesaures_male_modelF.RData")
D_RS.F<-D_RS.A
R2_D_RS.F<-R2_D_RS.A
HarrelsC.F<-HarrelsC.A

## Last, load model A, which doesn't need any names changing
load("discrim_mesaures_male_development_cohort_modelA_Tj_split_reanalysis.RData")
...
## First do R2_PM
setwd("B)
load("generate_discrim_measures_male_R2_PM.RData")
R2_PM.A<-unlist(R2_PM.A)
R2_PM.B<-unlist(R2_PM.B)
R2_PM.C<-unlist(R2_PM.C)
R2_PM.D<-unlist(R2_PM.D)

load("generate_discrim_measures_male_R2_PM_EandF.RData")
...
## Next I want to do UnoC
setwd("B)
load("generate_discrim_measures_male_UnoC.RData")
R2_PM.A<-unlist(R2_PM.A)
R2_PM.B<-unlist(R2_PM.B)
R2_PM.C<-unlist(R2_PM.C)
R2_PM.D<-unlist(R2_PM.D)

load("generate_discrim_measures_male_UnoC_EandF.RData")
R2_PM.E<-unlist(R2_PM.E)
R2_PM.F<-unlist(R2_PM.F)
...
### Next I want to do rho_w_a
setwd("B)
load("generate_discrim_measures_male_rho_wa.RData")
rho_wa.A<-unlist(rho_wa.A)
rho_wa.B<-unlist(rho_wa.B)
rho_wa.C<-unlist(rho_wa.C)
rho_wa.D<-unlist(rho_wa.D)

load("generate_discrim_measures_male_rho_wa_EandF.RData")
...
setwd("B)
load("generate_discrim_measures_male_IBS_modelA.RData")
...
load("generate_discrim_measures_male_IBS_modelB.RData")
R2_IBS<-vector("list",20)
for (i in 1:20){R2_IBS[[i]]<-(1-IBS[[i]][2]/IBS[[i]][1])}
for (i in 1:20){IBS[[i]]<-(IBS[[i]][2])}
...
load("generate_discrim_measures_male_IBS_modelC.RData")
R2_IBS<-vector("list",20)
for (i in 1:20){R2_IBS[[i]]<-(1-IBS[[i]][2]/IBS[[i]][1])}
for (i in 1:20){IBS[[i]]<-(IBS[[i]][2])}
...
load("generate_discrim_measures_male_IBS_modelD.RData")
R2_IBS<-vector("list",20)
for (i in 1:20){R2_IBS[[i]]<-(1-IBS[[i]][2]/IBS[[i]][1])}
for (i in 1:20){IBS[[i]]<-(IBS[[i]][2])}
...
save.image("performance_metrics_male.RData")

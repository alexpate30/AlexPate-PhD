##### This program will do calibration plots for male cohort, model B
##### This program will do calibration plots for male cohort, model B
##### This program will do calibration plots for male cohort, model B
...
rm(long_data_parallel_test, long_data_parallel)
...
create_risks<-function(model,datain){
  ...
  C<-model$var
  S<-model.matrix(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1 + agefrac2 + BMI + Atrialfib+ Atypical_antipsy_med+ 
                    Cholesterol_HDL_Ratio+ Corticosteroid_use + CKD345 + Erec_dysfunc + Ethnicity + Famhis_lstrict + Hypertension+ 
                    Migraine+ RA+ SBP+ SBP_std+ Sev_men_ill_qrisk+ Smoking+ T1dia+ T2dia+ Townsend+ SLE+
                    age.Atrialfib + age.Corticosteroid_use + age.CKD345 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                    age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 +
                    age.Townsend.3 + age.Townsend.4 + age.BMI + age.SBP, data=datain)
  S<-S[,-1]
  p<-(S%*%B)[,1]
...
setwd("B")
save.image("calibration_male_split.RData")
...
# Now plot
p<-ggplot(temp.dat) 
p+ geom_point(aes(x=xvals,y=value,shape=variable)) + scale_shape_manual(values=c(1,19)) + ggtitle("Male - All patients")
### In this program I will calculate UnoC for models E and F (female cohort)
...
create_lp<-function(model,frailties){
...
## First make the linear predictors
## Now I need to parallelise this and do for each dataset
pred.model.1 <- vector("list",10)
pred.model.1[[1]] <- create_lp(fit_data_parallel.1.coxph[[1]],frail.coeffs.all.1[[1]])
pred.model.1[[2]] <- create_lp(fit_data_parallel.1.coxph[[2]],frail.coeffs.all.1[[2]])
...
rm(list=setdiff(ls(),list("UnoC.E","UnoC.F")))
setwd("B")
save.image("generate_discrim_measures_female_UnoC_EandF.RData")
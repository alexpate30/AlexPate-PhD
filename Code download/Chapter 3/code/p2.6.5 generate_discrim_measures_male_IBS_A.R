##### This program will generate Integrate Brier Score using model A, male cohort
...
get.IBS<-function(in1,in2){temp<-pec(in1,formula=Surv(CVD_time,CVD_cens_R) ~ agefrac1 + agefrac2 + BMI + Atrialfib+ 
                                       Cholesterol_HDL_Ratio + CKD45 + Ethnicity + Famhis_lstrict + Hypertension+ 
                                       RA+ SBP+ Smoking+ T1dia+ T2dia+ Townsend+
                                       age.Atrialfib + age.CKD45 + age.Famhis_lstrict + age.Hypertension + age.T1dia + 
                                       age.T2dia + age.Smoking.1 + age.Smoking.2 + age.Townsend.1 + age.Townsend.2 + age.Townsend.3 +
                                       age.Townsend.4 + age.BMI + age.SBP, 
                                     data=mean.dat.test,
...
setwd("B")
save.image("generate_discrim_measures_male_IBS_modelA.RData")
print('model A saved')
